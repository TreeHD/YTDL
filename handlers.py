...
async def handle_quality_command(update: Update, context: ContextTypes.DEFAULT_TYPE, request_queue):
    ...
    status_msg = await context.bot.send_message(chat_id=chat_id, text=f"📥 Adding to queue ({max_height}p)...", reply_to_message_id=message_id)
    await request_queue.put((chat_id, url, message_id, max_height, status_msg))
    q_size = request_queue.qsize()
    await status_msg.edit_text(f"📥 Added to queue ({max_height}p). Queue Depth: {q_size}")
...
async def handle_playlist_command(update: Update, context: ContextTypes.DEFAULT_TYPE, playlist_queue):
    ...
    await playlist_queue.put((chat_id, url, message_id, max_height, status_msg))
    await status_msg.edit_text(f"📋 Playlist added to queue ({max_height}p). Sequential processing started...")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE, request_queue):
    ...
    if is_pl:
        ...
        await request_queue.put((chat_id, url, message_id, final_res, status_msg))
        q_size = request_queue.qsize()
        mode_str = "Audio M4A" if mode == 'audio' else f"{res}p"
        await status_msg.edit_text(f"📥 Added to queue ({mode_str}). Queue Depth: {q_size}")

# --- Subscription Handlers ---
async def handle_subscribe_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /subvideo command."""
    await _perform_subscribe(update, context, sub_type='video')

async def handle_subscribe_live(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /sublive command."""
    await _perform_subscribe(update, context, sub_type='live')

async def _perform_subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE, sub_type='video'):
    """Shared logic for subscriptions."""
    chat_id = update.effective_chat.id
    message_id = update.message.message_id
    ...
    if not context.args:
        cmd = "/subvideo" if sub_type == 'video' else "/sublive"
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"Usage: `{cmd} <channel_url> [quality]`\n\nExample:\n`{cmd} https://www.youtube.com/@ChannelName 1080`",
...
async def handle_unsubscribe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if success:
            ...
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"❌ No active {sub_type} subscription found.",
                reply_to_message_id=message_id
            )
    except Exception as e:
...
async def audio_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        file_path, title, video_id, thumb_path = await loop.run_in_executor(None, lambda: download_content(url, progress_cb, audio_only=True))
        ...
        reply_id = query.message.reply_to_message.message_id if query.message.reply_to_message else None
        ...
        # Delete status message upon completion
        try:
